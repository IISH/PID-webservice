...
/**
 * We make sure that clients can still use the service using the "oauth" keyword until we deprecate it.
 */
final public class OAuth2ProtectedResourceFilter extends GenericFilterBean {

    private static String DEPRECATED_BEARER_TYPE = "oauth";
    ...
    protected String parseHeaderToken(HttpServletRequest request) {
        ...
        while (headers.hasMoreElements()) {
            ...
            } else if ((value.toLowerCase().startsWith(DEPRECATED_BEARER_TYPE.toLowerCase()))) {
                authHeaderValue = value.substring(DEPRECATED_BEARER_TYPE.length()).trim();
            }
